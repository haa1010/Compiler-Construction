...
void compileBlock3(void)
{
  ...
  if (lookAhead->tokenType == KW_VAR)
  {
    ...
    {
      eat(TK_IDENT);
      // printf("check ident fresh compileBlock3 - %s\n", currentToken->string);

      checkFreshIdent(currentToken->string);
      // printf("%s %s\n",symtab->currentScope->owner->name, currentToken->string);
...
void compileParam(void)
{
  ...
  // printf("check ident fresh compileparam - %s\n", currentToken->string);
...
void compileForSt(void)
{
  ...
  // checkIntType(obj->varAttrs->type);
  // checkBasicType(obj->varAttrs->type);
  eat(SB_ASSIGN);
  type = compileExpression();
  checkTypeEquality(type, obj->varAttrs->type);
